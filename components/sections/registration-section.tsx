    ...
    <section id="register" className="py-24">
      <div className="container px-4 mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-semibold">Pilih Misa</h3>
                  <p className="text-sm text-muted-foreground">Choose an mass</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center">
                ...
                <div>
                  <h3 className="font-semibold">Pilih Kelompok</h3>
                  <p className="text-sm text-muted-foreground">Choose your kelompok</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Tri Hari Suci Registration</CardTitle>
              <CardDescription>Register for an upcoming Mass.</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <Label htmlFor="gatheringId">Misa</Label>
                  {isLoadingGatherings ? (
                    <div className="flex items-center gap-2 h-10 px-3 border rounded-md">
                      <Loader2 className="h-4 w-4 animate-spin" />
                      <span className="text-sm text-muted-foreground">Loading Misas...</span>
                    </div>
                  ) : (
                    <Select
                ...
                <div>
                  <Label htmlFor="groupId">Kelompok</Label>
                  {isLoadingGroups ? (
                    <div className="flex items-center gap-2 h-10 px-3 border rounded-md">
                      <Loader2 className="h-4 w-4 animate-spin" />
                      <span className="text-sm text-muted-foreground">Loading kelompoks...</span>
                    </div>
                  ) : (
                    <Select value={formData.groupId} onValueChange={handleGroupChange}>
                      <SelectTrigger>
                        <SelectValue placeholder="Pilhan kelompok" />
                      </SelectTrigger>
                      <SelectContent>
                        {groups.map((group) => (
                ...
                <div>
                  <Label htmlFor="memberId">Prodiakon</Label>
                  {isLoadingMembers ? (
                    <div className="flex items-center gap-2 h-10 px-3 border rounded-md">
                      <Loader2 className="h-4 w-4 animate-spin" />
                      <span className="text-sm text-muted-foreground">Loading prodiakons...</span>
                    </div>
                  ) : (
                    <Select
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue
                          placeholder={formData.groupId ? "Pilih nama anda" : "Pilh kelompok terlebih dahulu"}
                        />
                      </SelectTrigger>
                      <SelectContent>
